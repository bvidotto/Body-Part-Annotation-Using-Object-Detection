"""
File: dkdk.py
Authors: Not me, can't find the source as of Q4 2024
Date: ?
"""
...
def duckduckgo_scrape_urls(keywords: str, max_results: int,
  ...
  if os.path.exists(keywords + "/downloaded.txt"):
    with open(keywords + "/downloaded.txt", 'r') as f:
  ...
  while True:
      ...
      for result in data["results"]:
        if(img_size == ImgSize.Cached): 
          if result["thumbnail"] + '\n' not in downloaded:
            links.append(result["thumbnail"])
        else:
          if result["image"] + '\n' not in downloaded:
            links.append(result["image"])
...
def duckduckgo_search(path: Union[str, Path], label: str, keywords: str, max_results: int=100,
  ...
  results = download_urls(Path(path)/label, links, uuid_names=uuid_names)

  with open(keywords + "/downloaded.txt", 'a') as f:
    for link in links:
      f.write(link + "\n")
